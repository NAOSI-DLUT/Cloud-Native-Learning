...
​	比如K3S、红帽的OpenShift、Google的GDCE、爱立信的CCD、微软的AWS。
...
从硬件的角度：工作资源为Node。一个Node一般是机柜上的一台机器，也可以是一台虚拟机，但是是一台完整的电脑（区别于docker）。

从软件的角度：工作资源的调度单位为Pod。一个Pod类似是一个Node上的虚拟机，所以一个Node上可以有多个Pod。

​   容器在Pod里运行，一个Pod内可以运行一个或多个容器实例。一般会把功能结合比较紧密的容器放入同一个Pod，进而形成一个微服务，一个Pod只完成一个服务（功能）。
